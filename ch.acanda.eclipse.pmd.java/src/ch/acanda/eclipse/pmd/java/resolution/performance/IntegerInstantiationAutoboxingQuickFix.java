...
public class IntegerInstantiationAutoboxingQuickFix extends ASTQuickFix<ClassInstanceCreation> {
    ...
    public String getDescription() {
        ...
        if (matcher.find()) {
            return "Replaces the Integer instantiation with <b>" + matcher.group(1) + "</b>.";
        }
        return "Uses autoboxing instead of an explicit integer instantiation";
    }
    ...
    /**
     * Replaces the Integer instantiation with its argument, e.g. {@code new Integer(123 + x)} with {@code 123 + x}.
     */
    @Override
